...
| پکیج `@capacitor/android` + `splash-screen` + `status-bar` | ✅ نصب شده |
...
فایل `assets/icon.svg` آیکون برند فایننس‌پرو است (زمردی + نمودار طلایی). برای جایگزینی آیکون‌های پیش‌فرض اندروید:
...
| `Could not resolve all artifacts` / خطای `dl.google.com` | `node patch-android-mirror.cjs` بزنید (میرور Aliyun) یا از VPN استفاده کنید |
...
node patch-android-mirror.cjs  # فقط بار اول (میرور ایران)
...
npm run build
npx cap sync android
node patch-android-mirror.cjs   # اگر sync فایل‌های gradle را بازنویسی کرد
