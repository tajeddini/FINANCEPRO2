...
| آیکن | Lucide React |
| اکسل | ExcelJS |
| بک‌اند/دیتابیس | Supabase (PostgreSQL + REST) |
| میزبانی سایت | Vercel |
| ربات تلگرام | Node.js بدون وابستگی (Polling روی VPS) |
| اپ اندروید | Capacitor 8.5 |
| قابلیت‌های بومی اندروید | @capacitor-community/speech-recognition (دیکته و جست‌وجوی صوتی) · @capawesome/capacitor-file-picker (انتخاب فایل) · @capacitor/local-notifications (یادآورها) · @capacitor/filesystem + @capacitor/share (ذخیره/اشتراک فایل) · پلاگین محلی `capacitor-sms-reader` (خواندن پیامک) |
...
│   │   ├── tx-modal.tsx        ← فرم ثبت/ویرایش تراکنش + ماشین‌حساب + پیش‌پرکردن از پیامک بانکی
│   │   ├── dashboard.tsx       ← داشبورد + تحلیل رفتار خرج با تگ‌ها + ۵ پیامک بانکی اخیر
...
│   │   ├── manage.tsx          ← مدیریت ۸ تب CRUD + تب پیامک‌ها و برچسب‌ها
...
│       ├── themes.ts           ← تعریف ۷ تم رنگی، شامل شیشه‌ای تیره
...
├── plugins/capacitor-sms-reader/ ← پلاگین محلی اندروید برای مجوز و اسکن inbox پیامک
...
│   └── gen-icon-pngs.py        ← تولید PNG آیکون برای بیلد اندروید/CI
├── public/                     ← PWA: manifest + sw.js + icon.svg + آیکون‌های PNG
...
|---|---|
| **داشبورد** | موجودی/درآمد/هزینه با مخفی‌سازی جدا، نرخ پس‌انداز و نمودار روند موجودی، ثبت سریع (صوتی + تشخیص هوشمند)، حساب‌ها، فعالیت اخیر، تحلیل رفتار خرج با تگ‌ها، ۵ پیامک بانکی اخیر با وضعیت «ثبت شد» و بازکردن فرم پیش‌پرشده برای پیامک‌های ثبت‌نشده |
| **روزانه** | خرج و درآمد امروز، برنامهٔ امروز با تیک، قرارهای امروز، و یادآوری‌های هفت روز آینده برای بدهی، بودجه و تمدید اشتراک |
| **تراکنش‌ها** | فیلتر نوع/دسته/حساب/روش پرداخت + بازهٔ زمانی (۹ آماده + دلخواه)، جست‌وجو و تگ‌ها، ویرایش، حذف با بازگشت ۳۰ ثانیه، ورود CSV/XLSX، خروجی CSV، ثبت و بررسی پیامک بانکی؛ prefill مبلغ/تاریخ/نوع با تبدیل Rial به Toman |
...
| **گزارش‌ها** | نمودار ۶ ماهه + پیش‌بینی ماه بعد، امتیاز سلامت، نقشه حرارتی ۱۴ هفته، نشان‌ها، مقایسه ماهانه، **گزارش هوشمند برای کپی و تحلیل با هوش مصنوعی**، خروجی اکسل چندبرگی/CSV/چاپ/PDF |
| **مدیریت** | ۸ تب: حساب، دسته با آیکون، تگ، بودجه، هدف، تراکنش دوره‌ای، روش پرداخت و «پیامک‌ها»؛ تب پیامک‌ها فهرست کامل پیامک‌های اسکن‌شده، فیلتر بازهٔ شمسی، وضعیت ثبت و حذف محدود به پیامک‌های ثبت‌شده، پیش‌پرکردن پیامک‌های ثبت‌نشده، انتخاب و ذخیرهٔ «اسکن از تاریخ» شمسی و اسکن پیامک‌های قدیمی را دارد + داده نمونه/پاک‌سازی |
| **تنظیمات** | تم روشن/تیره و ۷ تم رنگی، پین و قفل فوری، سینک ابری + تست اتصال/سینک دستی، مجوز فعال‌سازی پیامک بانکی اندروید (بدون تکرار کنترل‌ها و تاریخ اسکن/اسکن تاریخی در این صفحه)، یادآور بومی، هوش مصنوعی، ربات تلگرام، پشتیبان/بازیابی، انتقال بین مرورگر، فهرست کاربران و حذف حساب |
...
۷ تم ترکیبی، شامل تم شیشه‌ای تیره (Glassmorphism Dark)، همگی با CSS Variables (`--fp-*`) و انتخاب از تنظیمات یا دکمهٔ پالت در نوار بالا:
- زمرد و طلا (پیش‌فرض) · زمردی دارک · زعفرانی و فیروزه · یاقوتی و صورتی · آبی سلطنتی و آسمان · مسی و زیتونی · **شیشه‌ای تیره**
...
- [x] **خواندن پیامک‌های بانکی در اندروید** (مجوز خواندن/دریافت، اسکن تاریخچه و صف بررسی قبل از ثبت — رسالت و بانک ملی)
- [x] **پیش‌پرکردن و رهگیری پیامک بانکی:** لمس پیامک، فرم تراکنش را با مبلغ تومانی، تاریخ و نوع پر می‌کند؛ پس از ثبت، پیامک با وضعیت «ثبت شد» نگه‌داری می‌شود و در داشبورد و تب پیامک‌های مدیریت قابل مشاهده است
- [x] **محدودسازی اسکن پیامک:** تاریخ شروع اسکن شمسی در تب «پیامک‌ها»ی مدیریت ذخیره می‌شود و هم اسکن تاریخی صندوق ورودی و هم پیامک‌های آینده فقط از آن تاریخ به بعد بررسی می‌شوند
- [x] **تکمیل مکان‌یابی پیامک‌ها:** بخش «پیامک‌های بانکی» و فیلتر/اسکن تاریخ در صفحهٔ مدیریت متمرکز شده‌اند و از صفحهٔ تنظیمات حذف شده‌اند تا دوباره‌کاری و تکرار UI حذف شود
- [x] **رفع نمایش اشتباه تاریخ پیامک‌ها:** تاریخ‌های پیامک‌های بانکی با تبدیل درست شمسی→میلادی بر اساس تقویم جلالی، نه فرض‌کردن ماه به‌صورت میلادی، نمایش داده می‌شوند و ماه‌ها دیگر اشتباه به خرداد/بازهٔ نادرست نمی‌روند
...
- [ ] جست‌وجوی سراسری صوتی پیشرفته‌تر (نسخهٔ فعلی جست‌وجوی صوتی وب و اندروید را دارد)
...
| v1.37 | **همگام‌سازی سند با وضعیت واقعی کد:** مستندسازی پلاگین محلی `capacitor-sms-reader` و قابلیت مجوزدهی/اسکن پیامک بانکی اندروید (رسالت و بانک ملی) با صف بررسی قبل از ثبت؛ اصلاح شمارش تم‌ها به ۷، مدیریت به ۷ تب واقعی، و Supabase به REST؛ تکمیل جزئیات داشبورد، روزانه، گزارش‌ها، تنظیمات، خروجی چندبرگی و فرمان‌های ربات |
| v1.38 | **تکمیل جریان پیامک بانکی:** تبدیل مبلغ Rial به Toman در parser و فرم، پیش‌پرکردن نوع/مبلغ/تاریخ و حساب هنگام لمس پیامک، نگه‌داری پیامک پس از ثبت با `status` و `transactionId` به‌جای حذف، نمایش ۵ پیامک اخیر و نشان «ثبت شد» در داشبورد، و افزودن بخش «پیامک‌ها» در مدیریت با فهرست کامل، فیلتر بازهٔ شمسی، prefill پیامک‌های ثبت‌نشده و حذف محدود به پیامک‌های ثبت‌شده؛ وزن اعداد نمایشی نیز به حالت پررنگ قبلی بازگردانده شد و تاریخ شروع اسکن شمسی برای اسکن تاریخی و آینده اضافه شد |
